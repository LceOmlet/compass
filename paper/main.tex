...
instructions, and agent skills, which we collectively call textual programs.
A fixed rollout budget must support both
candidate revision and the choice of where to search next.  \method{} uses
execution results to choose which candidates to revise and which batches to
use for those revisions.  It compares
candidates on shared instances and updates their relative credit as new results
arrive.  A round-robin queue gives the highest-credit candidates repeated
proposal opportunities, and maximum-weight matching assigns their proposal
slots to batches using their recorded score gaps.  The method tracks
the examples used to create each candidate and its ancestors.  Each fixed
revision is evaluated on a separately sampled batch outside that history.
Proposal generation and admission evaluation share the rollout budget.
...
(PUPA).  Relative to
published GEPA, the largest gain is $36.7$ percentage points on AIME-2025, and
the mean gain across all six shared benchmarks is $11.8$ points.
\end{abstract}
...
as instance frontiers change.  Round-robin selection allocates proposal turns
to the highest-credit candidates, including cutoff ties.  Maximum-weight
...
\begin{enumerate}[leftmargin=*,itemsep=2pt,topsep=3pt]
  ...
  \item Round-robin parent selection and maximum-weight matching allocate
  proposal work using recorded score gaps; paired admission evaluates
...
COMPASS selects proposal parents by round-robin traversal of the active set.
The set is ordered by decreasing $G_s$, with earlier candidate index breaking
exact ties, and the cursor is carried across waves.  For a fixed order
...
in the pool.  Round-robin selection and maximum-weight matching direct the next
proposal wave; paired admission evaluates the resulting revisions on separate
examples.  Under the reported budgets, COMPASS achieves the highest six-task
...
instance-wise credit conservation, finite-pool selection bounds, cyclic
slot coverage, and optimality of the proposal assignment.
