...
At optimization step $t$, reuse the credit-eligible set
...
\end{gathered}
\]
An $N_x=1$ observation remains in the cache but does not enter the aggregate.
Whenever a new finite observation changes $\mathcal C_t(x)$ or $\F_t(x)$,
COMPASS recomputes $N_x$, $K_x$, and every affected candidate contribution on
...
governs which cached observations may contribute to candidate $s$'s relative
credit, and $X(s,P)$ extends that exclusion to the prospective child's admission.

\section{Algorithmic Details}
\label{app:algorithm}

\subsection{Proposal matching}

\Cref{eq:repairable_gap,eq:proposal_assignment} define the
proposal objective.  COMPASS builds the complete slot--batch
weight matrix from the cache and solves the resulting
...
Proposal tasks with usable execution traces and reflection data proceed to child
generation.  An optional step skips tasks whose parent attains the maximum
score on every proposal instance.  Let $m\le w$ be the
number of remaining tasks.  For each task independently, the benchmark evaluator
samples one batch $A_i$ from that task's admission sampler after removing every
...
parent when no frontier candidate has defined credit.  Missing reference
observations are evaluated before child generation and stored using the
highest-score cache rule.  The admission instances, references, and outcomes
remain hidden from the generator.  The fixed child is admitted exactly when the
paired aggregate in \Cref{eq:admission_gate} is positive.

\subsection{Wave execution}

Let $\mathcal H_t$ contain the lineage, proposal batches, cache,
remaining budget, and position in the active-set order.  A new observation replaces an
existing score and its associated output only when its score is strictly higher.
Each observed candidate--instance pair therefore retains one record.

\begin{algorithm}[H]
\caption{One \method{} wave}
\label{alg:compass_wave}
\begin{algorithmic}[1]
\Require search state $\mathcal H_t$, wave width $w$, active cutoff $\rho$
\Ensure updated search state $\mathcal H_{t+1}$
\State Recompute every defined $G_s$ from the cache
...
\EndFor
\State Optionally skip tasks with maximal parent scores on every proposal instance
\State $\mathcal T\gets$ remaining tasks with usable traces and reflection data
...
\ForAll{$(s_i,P_i)\in\mathcal T$ \textbf{in parallel}}
  ...
  \If{$\Delta_i>0$}
    \State Add $p_i$ to the candidate pool and its eligible admission evidence to the cache
  \EndIf
\EndFor
\State Update the remaining budget, frontiers, credits, and position in the active-set order
\end{algorithmic}
\end{algorithm}

Repeat \Cref{alg:compass_wave} while cumulative proposal and evaluation cost
remains below $B$, and complete the final wave before stopping.
...
\subsection{Properties of parent selection and proposal matching}
...
Conditional on the $w$ drawn parent slots and $w$ proposal batches,
\Cref{eq:proposal_assignment} maximizes total matching weight over all
one-to-one slot--batch assignments.
\end{proposition}

\begin{proof}[Proof of maximum-weight proposal matching]
The $w$ parent slots and $w$ proposal batches define a complete bipartite graph
...
each remaining proposal task.  The selected program is evaluated once.

The Qwen3-8B runs select candidates 55, 78, 34, and 28 for HotPotQA,
IFBench, PUPA, and LiveBench-Math, respectively.

\subsection{Official evaluation protocol}

The selected candidate is fixed at the end of optimization, before its test
scores are observed, and evaluated on the complete test split.  IFBench reports
the mean score over 294 instances.  AIME uses all 150 benchmark problems.
HoVer uses 300 instances with binary scores, and LiveBench-Math uses its
frozen 126-problem test with the official aggregation rule.
